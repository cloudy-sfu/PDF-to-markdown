...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Controls wrapped in a widget to allow enabling/disabling at once
        ...
        self.table.setHorizontalHeaderLabels(
            ["Input File", "Output Directory", "Page Ranges", "Layout", "Status"])
    ...
    def on_cell_double_clicked(self, row, col):
        if col == 0:
            ...
            dir_path = QFileDialog.getExistingDirectory(self, "Select Output Directory", self.table.item(row, col).text())
            if dir_path:
    ...
    def start_processing(self, ignore_failed=False):
        ...
        for row in range(self.table.rowCount()):
            ...
            if not input_file or not output_dir:
                logging.error(f"Task on row {row + 1} failed: Empty input or output paths.")
        ...
        if not valid_tasks:
            logging.info("No pending task to run.")
            return
    ...
    def skip_current(self):
        if self.worker is not None and self.worker.isRunning():
            ...
            logging.info("Terminated current job.")
    ...
    def stop_all(self):
        if self.worker is not None and self.worker.isRunning():
            ...
            if hasattr(self, 'current_running_row') and self.current_running_row is not None:
                self.set_row_status(self.current_running_row, "Failed", QColor("red"))
                self.current_running_row = None
            logging.info("Terminated all jobs.")
